...
public:
  ...
  render_frame(const Template &tmpl, double time,
               QualityLevel quality = QualityLevel::Final);

  /// Asynchronous rendering with future
  [[nodiscard]] std::future<FrameResult>
  render_frame_async(const Template &tmpl, double time,
                     QualityLevel quality = QualityLevel::Final);

  /// Deterministic rendering with hash validation
  [[nodiscard]] std::expected<FrameResult, RenderError>
  render_frame_deterministic(const Template &tmpl, uint64_t frame_number,
                             const FrameHash &expected_hash);
...
public:
  ...
  virtual void dispatch_compute(std::string_view kernel_name,
                                std::span<const uint8_t> params,
                                uint32_t work_groups_x, uint32_t work_groups_y,
                                uint32_t work_groups_z) = 0;

  /// Create GPU buffer
  [[nodiscard]] virtual uint64_t create_buffer(size_t size,
                                               bool host_visible) = 0;
  ...
  virtual void upload_buffer(uint64_t handle,
