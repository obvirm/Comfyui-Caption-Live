...
impl<'a> TextMeasurer for RustTypeMeasurer<'a> {
    fn measure_text(&self, text: &str, font_size: f64) -> (f64, f64) {
        ...
        let mut width = 0.0;
        let glyphs: Vec<_> = self.font.layout(text, scale, rusttype::point(0.0, 0.0)).collect();
        if let Some(last) = glyphs.last() {
            width = last.position().x as f64 + last.unpositioned().h_metrics().advance_width as f64;
        }
